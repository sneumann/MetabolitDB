massage_data <- function(data) {
  temp_data=data
  #  updateProgressBar(session, dataId = "uploadprogress", visible=TRUE, value=10)
...
#  if(any(is.na(select))){
#    
#    if(   (data$system_upload=="")   &   !(     (all(!is.na(select[c(1:4)])))   |   (  all(!is.na(select[c(1:3,5)]))  )           )                        ){
...
#    }
#    
#    if(   (!(data$system_upload==""))  &  !(      (all(!is.na(select[c(1,2,4)])))  |  (all(!is.na(select[c(1,2,5)])))      )  ){
...
#  if(any(unlist(   lapply(errors,function(x) x$error==1)    ))){    return(list(data=temp_data,errors=errors))    }
  ...
  #updateProgressBar(session, dataId = "uploadprogress", visible=TRUE, value=15)
  ...
  # updateProgressBar(session, dataId = "uploadprogress", visible=TRUE, value=85)
  ...
  #updateProgressBar(session, dataId = "uploadprogress", visible=TRUE, value=95)
  ...
  #updateProgressBar(session, dataId = "uploadprogress", visible=TRUE, value=100)
